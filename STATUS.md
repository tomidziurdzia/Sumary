...
| **Bonus: inline edit**                         | Done – `updateTransaction` Server Action, `useMutation` + optimistic update, inline edit in `TransactionRow` (Edit / Save / Cancel).                          |
| **Bonus: RLS**                                 | Done – `user_id` column, RLS enabled on `public.transactions`, policies so users only see/edit their own rows.                                                |

---

## Not done yet

Nothing – all assessment requirements and bonuses are complete.
